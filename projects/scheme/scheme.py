...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        prc = scheme_eval(first, env)
        check_procedure(prc)
        l1 = lambda f,e: lambda x: f(x,e)
        l2 = l1(scheme_eval,env)
        operands = rest.map(l2)
        return scheme_apply(prc, operands, env)     
        # END PROBLEM 5
    ...
    def make_child_frame(self, formals, vals):
        ...
        def help(c,f,v):
        	if f is nil and v is nil:
        		return c
        	elif f is nil and v is not nil:
        		raise SchemeError('vals has too many args')
        	elif f is not nil and v is nil:
        		raise SchemeError('vals has too few args')
        	c.define(f.first,v.first)
        	return help(c,f.second,v.second)       	

        return help(Frame(self),formals,vals)
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        # END PROBLEM 4
        	raise SchemeError('wrong amount of arguments')
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        new_expr = Pair(target.second, expressions.second)
...
def do_and_form(expressions, env):
    ...
    if scheme_truep(f) and expressions.second is not nil:
    	return do_and_form(expressions.second,env)
    elif scheme_truep(f) and expressions.second is nil:
    	return f
    elif scheme_falsep(f):
    	return False
    # END PROBLEM 13
...
def do_or_form(expressions, env):
    ...
    if expressions is nil:
    	return False
    f = scheme_eval(expressions.first,env)
    if scheme_falsep(f) and expressions.second is not nil:
    	return do_or_form(expressions.second,env)
    elif scheme_falsep(f) and expressions.second is nil:
    	return False
    elif scheme_truep(f):
    	return f
    # END PROBLEM 13
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if scheme_truep(test):
            # BEGIN PROBLEM 14
            if scheme_truep(clause.first) and clause.second is not nil:
            	return eval_all(clause.second,env)
            elif scheme_truep(clause.first) and clause.second is nil:
            	return test
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    f = lambda x: x.first
    s = lambda x: scheme_eval(x.second.first,env)
    c = lambda x: check_form(x,2,2)

    check_formals(formals)
    bindings.map(c)
    formals = bindings.map(f)
    vals = bindings.map(s)
