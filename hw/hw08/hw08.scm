...
(define (larger-values x lst)
    (cond ((null? lst) nil)
          ((> (car lst) x) (append (list (car lst)) (larger-values x (cdr lst))))
...
(define (longest-increasing-subsequence lst)
    ...
    (if (null? lst)
        ...
        (begin
            ...
            (define with-first
                (append first (longest-increasing-subsequence (large-values-rest)))

            (define without-first
                (longest-increasing-subsequence (large-values-rest)))

            (if (null? large-values-rest)
                without-first
                with-first))))
    )
...
(define (derive-sum expr var)
  (make-sum (derive (augend expr) var) (derive (addend expr) var))
)
