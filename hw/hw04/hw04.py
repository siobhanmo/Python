...
def prune_leaves(t, vals):
    ...
    """
    def helper(t):
        if is_leaf(t) and label not in vals:
            return tree(label(t))
        return tree(label(t), [helper(b) for b in branches(t) if not (is_leaf(b) and label(b) in vals)])
...
def balanced(m):
    ...
    if is_weight(m):
        return True
    elif length(right(m))*total_weight(end(right(m))) == length(left(m))*total_weight(end(left(m))):
        return balanced(end(right(m))) == balanced(end(left(m)))
    else:
        return False
